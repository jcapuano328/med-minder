    ...
    render() {
        return (
            <View style={{flex:1, flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'flex-start'}}>
                <Text style={{flex: 1, fontSize: 16, fontWeight: 'bold', fontStyle: 'italic'}}>{this.props.name}</Text>
                ...
                {this.props.meds.map((d, i) => {
                    ...
                    return (
                        <View key={i} style={{flex: 1, flexDirection: 'row'}}>
                            <Text style={{flex: 1, fontWeight: 'bold'}}>{med.name} {med.dosage}</Text>
                            <Text style={{flex: 2}}>{med.instructions}</Text>
                        </View>
                    );
                })}
                </View>
